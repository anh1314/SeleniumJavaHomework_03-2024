...
public class LoginPage {
      ...
      private By alertMessage = By.xpath("//div[@role='alert']");//div[@role='alert']
      ...
      private void verifyLoginSucess() {
            Assert.assertFalse(DriverManager.getDriver().getCurrentUrl().contains("login"), "Login không thành công");
      }

      public void verifyLoginFail() {
            WebUI.checkElementDisplayed(alertMessage);
            Assert.assertTrue(DriverManager.getDriver().getCurrentUrl().contains("login"), "Fail, không ở trang Login");
